...
void Stage::Load()
{
	...
	top->SetPosition(glm::vec3{ 0.0f, 1.0f, 0.0f });
	top->SetScale(glm::vec3{ 5.0f, 1.0f, 5.0f });
	mPlanes.emplace_back(top);

	Plane* left{ new Plane{Object::kRed} };
	left->SetZRotate(true);
	left->SetZRotation(90.0f);
	left->SetPosition(glm::vec3{ -2.5f, 0.5f, 0.0f });
	left->SetScale(glm::vec3{ 1.0f, 1.0f, 5.0f });
	mPlanes.emplace_back(left);

	Plane* right{ new Plane{Object::kGreen} };
	right->SetZRotate(true);
	right->SetZRotation(-90.0f);
	right->SetPosition(glm::vec3{ 2.5f, 0.5f, 0.0f });
	right->SetScale(glm::vec3{ 1.0f, 1.0f, 5.0f });
	...
	Plane* front{ new Plane{Object::kBlack} };
	front->SetXRotate(true);
	front->SetXRotation(-90.0f);
	front->SetPosition(glm::vec3{ 0.0f, 0.5f, 2.5f });
	front->SetScale(glm::vec3{ 5.0f, 1.0f, 1.0f });
